...
const Edit = () => {
  ...
  const handleUpdate = async () => {
    ...
    const updateQuery = await fetch("https://rpl-backend-production.up.railway.app/v1/property/faq/" + router.query.id, {
      method: "PATCH",
      headers: {
        Authorization: token,
  ...
  useEffect(() => {
    ...
    fetch("https://rpl-backend-production.up.railway.app/v1/property/" + router.query.id, {
      method: "GET",
      headers: {
        "Authorization": token
      }
  ...
  return <>
    <Template>
      <main className="min-h-screen px-14 py-5 bg-[#FFF6F6]">
        ...
        <h1 className="text-[#F875AA] font-extrabold text-5xl mb-20 text-center">Update FAQ</h1>
        <form className="w-2/3 mx-auto space-y-10 flex flex-col align-middle justify-evenly" onSubmit={(e) => {
          ...
          <div className="flex flex-row align-middle justify-between">
            ...
            <input disabled value={infoPerusahaan.question} type="number" required className="drop-shadow-xl w-2/3 p-2 rounded-xl" />
          </div>
          <div className="flex flex-row align-middle justify-between">
