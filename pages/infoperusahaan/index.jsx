...
const Index = () => {
  ...
  const getNextPage = async (page) => {
    ...
    const getItems = await fetch("https://rpl-backend-production.up.railway.app/v1/property" + (page), {
      method: "GET",
      headers: {
        Authorization: token
      }
  ...
  const handleDelete = async () => {
    ...
    const deleteRequest = await fetch("https://rpl-backend-production.up.railway.app/v1/faq/delete/" + deleteToggle, {
      method: "DELETE",
      headers: {
        Authorization: token
      }
    ...
    <Template>
      <main className="min-h-screen px-14 py-5 bg-[#FFF6F6]">
        ...
        <div className="flex flex-row align-middle justify-between">
          <h1 className="text-[#F875AA] font-extrabold text-5xl mb-8">Informasi Perusahaan</h1>
          <button onClick={(e) => {
            e.preventDefault();
            router.push("/instruktur/create")
          }} className="bg-[#F875AA] p-4 text-lg font-bold text-white rounded-3xl">Create FAQ</button>
        </div>
        <table className="w-full text-center border-spacing-3 border-separate">
          ...
          <tbody>
            {infoPerusahaan.map((faq, index) => {
              return <tr key={index}>
                <td className="p-6 border border-[#F875AA] bg-white">{faq.question}</td>
                <td className="p-6 border border-[#F875AA] bg-white">{faq.answer}</td>
                <td className="px-2 flex flex-col align-middle justify-evenly space-y-2 ">
                  <button className="bg-[#AEDEFC] p-1 rounded-lg" onClick={(e) => {
                    e.preventDefault();
                    router.push("/property/faq/update/" + index)
                    return;
                  }}>Update</button>
      ...
      {deleteToggle !== null && <div className="left-0 top-0 fixed w-screen h-screen bg-white bg-opacity-80 p-20 flex flex-col align-middle justify-center">
        <div className="w-1/2 mx-auto space-y-5">
          <h1 className="text-center p-10 bg-[#FFDFDF] border-2 border-[#F875AA] rounded-xl font-bold text-xl">Apakah anda yakin akan menghapus data FAQ?</h1>
